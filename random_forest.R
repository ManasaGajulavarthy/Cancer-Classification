...
random_forest_alldata <- randomForest(Tumor.Class ~ .,data = train_alldata,importance=TRUE)
pred_alldata = predict(random_forest_alldata, newdata=test_alldata[,-1], type = "class")
...
predictions <- as.data.frame(predict(random_forest_alldata, test_alldata, type = "prob"))
...
#Run random forest model and predict tumor class on test data
random_forest_reduceddata <- randomForest(Tumor.Class ~ .,data = train_reduceddata)
pred_reduceddata = predict(random_forest_reduceddata, newdata=test_reduceddata[,-1], type = "class")
...
#Predicting probabilities of test data for each class
predictions_r <- as.data.frame(predict(random_forest_reduceddata, test_reduceddata, type = "prob"))
#Taking maximum probability for the sample and assign to predict
